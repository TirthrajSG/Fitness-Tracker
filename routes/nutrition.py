...
def index():
    ...
    logs = FoodLog.query.filter_by(date=the_date).order_by(FoodLog.meal).all()
    logs_by_meal = {m: [l for l in logs if l.meal == m] for m in MEALS}
    totals = na.daily_totals(logs)

    week_start = the_date - timedelta(days=6)
    week_logs = FoodLog.query.filter(FoodLog.date >= week_start, FoodLog.date <= the_date).all()
...
def log_food():
    try:
        ...
        if food_id:
            food = Food.query.get_or_404(int(food_id))
            factor = quantity / food.serving_size if food.serving_size else quantity
            log = FoodLog(
                date=the_date, meal=meal, food_id=food.id, food_name=food.name, quantity=quantity,
            ...
            log = FoodLog(
                date=the_date, meal=meal, food_name=request.form.get("food_name", "Custom entry").strip(),
